...
describe("Chat panel tests", () => {
   ...
   test("Show list when there is active users", async () => {
      ...
      activeUsers.forEach((activeUser, index) => {
         ...
         expect(activeUser).toContainElement(
            screen.getByAltText(mockActiveUsers[index].name)
         );
      });
   });
   ...
   test("Start talk with another users", async () => {
      ...
      expect(receptor).toHaveTextContent(
         mockActiveUsers[randomActiveUser].name
      );
   });
});
